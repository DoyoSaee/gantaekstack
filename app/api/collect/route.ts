...
async function run() {
  ...
  results.push(await runCollection(jobicySource, 60, 800));
  results.push(await runCollection(hnSource, 80, 800)); // 최신 스레드 신규분만(멱등)
  results.push(await runCollection(museSource, 60, 800));
